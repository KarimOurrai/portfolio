...
export function getContentByPath(targetPath : string) {
...
export async function getBlogPosts() {
  ...
  const data = await Promise.all(
    postsDirectory.map(async (subDir) => {
      const post :  any = await getContentByPath(path.join("blog/posts", subDir));
...
export async function getSinglePost(title : string) {
	const blogs : any = await getContentByPath('blog');
	const targetedPost = blogs.filter(
		(blogPost : any) => blogPost.title.replace(/[^A-Z0-9]+/gi, '-') === title
	);
